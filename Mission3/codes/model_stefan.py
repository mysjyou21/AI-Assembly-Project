...
def METRIC(x, class_num, name='image', reuse=False, is_train=False, regularizer=None):
    with tf.variable_scope(name+'_METRIC', reuse=reuse):
        ...
        x = fc(x, class_num, 'fc5', norm=None, activation=None, is_train=is_train, regularizer=regularizer)
...
def TRANSFORM(x, class_num, name='trans', reuse=False, regularizer=None):
...
#        x = fc(x, 128, 'fc4', activation='tanh')
        x = fc(x, class_num, 'fc1', activation='lrelu', regularizer=regularizer)
        x = fc(x, class_num//2, 'fc2', activation='lrelu', regularizer=regularizer)
#        x = fc(x, class_num//2, 'fc3', activation='lrelu', regularizer=regularizer)
        x = fc(x, class_num, 'fc3', activation=None)

        return x

def DISCRIMINATOR(x, class_num, name='discriminator', reuse=False, regularizer=None):
    with tf.variable_scope(name, reuse=reuse):
        x = fc(x, class_num//2, 'fc1', activation='lrelu', regularizer=regularizer)
