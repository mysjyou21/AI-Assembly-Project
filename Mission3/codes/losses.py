...
def cross_mean_discrepancy_loss(trans_feature, model_feature, trans_label, model_label, C, K):
    """ trans_feature: (CxK),channel """
    # CxKxchannel
    trans_feature = tf.reshape(trans_feature, [C, K, trans_feature.shape[1]])
    model_feature = tf.reshape(model_feature, [C, K, model_feature.shape[1]])
