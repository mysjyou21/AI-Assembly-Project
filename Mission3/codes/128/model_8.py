...
def METRIC(x, name='image', reuse=False, is_train=False, regularizer=None):
    with tf.variable_scope(name+'_METRIC', reuse=reuse):
        x = fc(x, 1024, 'fc1', norm='batch', activation='relu', is_train=is_train, regularizer=regularizer)
        x = fc(x, 512, 'fc2', norm='batch', activation='relu', is_train=is_train, regularizer=regularizer)
        x = fc(x, 256, 'fc3', norm='batch', activation='relu', is_train=is_train, regularizer=regularizer)
        x = fc(x, 128, 'fc4', norm='batch', activation='relu', is_train=is_train, regularizer=regularizer)
        x = fc(x, 8, 'fc5', norm=None, activation='tanh', is_train=is_train, regularizer=regularizer)
...
#        x = fc(x, 128, 'fc4', activation='tanh')
        x = fc(x, 8, 'fc1', activation='relu', regularizer=regularizer)
        x = fc(x, 4, 'fc2', activation='relu', regularizer=regularizer)
        x = fc(x, 8, 'fc3', activation='tanh', regularizer=regularizer)
#        x = fc(x, 16, 'fc4', activation='tanh')

        return x

def DISCRIMINATOR(x, name='discriminator', reuse=False, regularizer=None):
    with tf.variable_scope(name, reuse=reuse):
        x = fc(x, 4, 'fc1', activation='relu', regularizer=regularizer)
        x = fc(x, 1, 'fc2', regularizer=regularizer)

        return x

def VIEWS(views, name='view', _type='mean', reuse_t=False, is_train=False, regularizer=None):
