...
from model_stefan import CNN, METRIC, VIEWPOOL, TRANSFORM, VIEWS, DISCRIMINATOR
from losses import *
from dataset_stefan import Dataset
...
class DCA(object):
    ...
    def build_model(self, img, views):
        ...
        img_feature_logit = METRIC(img_feature_logit, class_num, name='image', is_train=self.is_train, regularizer=self.regularizer)
        with tf.variable_scope('image_METRIC'):
            img_feature = tf.nn.tanh(img_feature_logit)

        # img2view transformation(generator)
        trans_feature_logit = TRANSFORM(img_feature_logit, class_num, name='trans', regularizer=self.regularizer)
        ...
        view_feature_logit = METRIC(view_feature, class_num, name='view', is_train=self.is_train, regularizer=self.regularizer)
        with tf.variable_scope('view_METRIC'):
            view_feature = tf.nn.tanh(view_feature_logit)

        # discriminator  
        real_logit = DISCRIMINATOR(view_feature_logit, class_num, reuse=False, regularizer=self.regularizer)
        fake_logit = DISCRIMINATOR(trans_feature_logit, class_num, reuse=True, regularizer=self.regularizer)
...
#        self.lr = tf.train.exponential_decay(self.initial_lr, step, self.decay_steps, self.decay_rate, staircase=True)
    ...
    def train(self):
        ...
        # loss
        img_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=tf.one_hot(img_lab, depth=self.DataProvider.class_len), logits=imgf_logit))
        view_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=tf.one_hot(view_lab, depth=self.DataProvider.class_len), logits=viewf_logit))
        trans_class_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=tf.one_hot(img_lab, depth=self.DataProvider.class_len), logits=transf_logit))
...
#        trans_triplet_loss = triplet_loss(transf_logit, img_lab, self.margin)
        ...
        trans_loss = trans_class_loss + gen_loss*int(self.mode==0) + cmd_loss
...
#        dis_loss += tf.contrib.layers.apply_regularization(self.regularizer, dis_reg_vars)
        ...
        accurate = tf.argmax(tf.math.softmax(transf_logit, axis=1), axis=1)
        ...
        l5 = tf.summary.scalar('trans_class_loss', trans_class_loss)
...
#                        print(ac, ':::', b_imgs_lab)
    ...
    def test(self):
        self.max_epoch = 1
        BatchProvider = self.DataProvider.test_batch(size=(self.H, self.W), mode='pos')
        ...
        predict = tf.nn.softmax(transf_logit, axis=1)
        ...
        if self.testmode==4:
            test_f1 = normalize_feature(test_f1)
            test_f2 = normalize_feature(test_f2)
        ...
        with tf.Session(config=config) as sess:
            ...
            total_acc = 0
            total_num = 0
            steps = self.DataProvider.class_len if self.testmode==4 else self.DataProvider.img_len//self.DataProvider.class_len
#            steps_for_epoch = int((self.DataProvider.whole_batch_size-self.C*self.K+1)/self.C/self.K)
            ...
            while global_step < steps: #epoch <= self.max_epoch:
                b_imgs, b_views, b_imgs_lab, b_views_lab, b_pos_lab = next(BatchProvider)
                feed_dict = {img: b_imgs,
                            img_lab: b_imgs_lab,
                            view: b_views,
                            view_lab: b_views_lab}
                ...
                if self.testmode == 0: # Trans-View matching
                    ...
                    if global_step%5==0:
                        print(indices)
                    ac, num = self.accuracy(b_imgs_lab, b_views_lab, indices)
                elif self.testmode == 4: # pos matching
                    b_transf, b_viewf = sess.run([test_f1, test_f2], feed_dict=feed_dict)
                    b_transf = np.tile(b_transf, [b_viewf.shape[0], 1])
                    dist = b_transf - b_viewf
                    dist = np.square(dist)
                    ...
                    th = theta[(index//12)%5]
                    ph = phi[index%12]
                    filename = self.DataProvider.class_numtoidx[b_views_lab[0]]
                    print(filename + 'matches alpha %d, theta %d, phi %d' % (al, th, ph))
                    print(dist)

                    # save img
                    pos_imgs = b_views[index]
                    name = self.DataProvider.class_numtoidx[b_views_lab[0]]
                    view_filenames = self.DataProvider.view_filenames[filename]
                    view_filenames = view_filenames[index*self.DataProvider.view_num:(index+1)*self.DataProvider.view_num]
                    for i in range(len(pos_imgs)):
                        pos_img = (pos_imgs[i]*255).astype(np.uint8)
                        cv2.imwrite(FIG_DIR+'/'+view_filenames[i]+'.jpg', pos_img)
                else: # Classification
                ...
                if self.testmode == 0:
                    ...
                    colors = ["red", "green", "blue", "yellow", "pink", "brown", "purple", "magenta", "violet", "black", "gray"]
                    b_ind = {}
                    self.C = self.DataProvider.class_len
