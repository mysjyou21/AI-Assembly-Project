...
def serial_detect(self, cut, cut_number):
    """
    ...
    # self.circles_loc[cut_number] = self.step_circles
    ...
    H, W, _ = img.shape
    if PRINT_PROGRESS:
        print('cut num %d : serial detect' % cut_number)

    # remove redundent bottom image part
    cv.rectangle(img, (0, int(15/16*H)), (W-1, H-1), (255, 255, 255), -1)


    # remove rectangle bubble area (fill white)
    if REMOVE_RECTANGLE:
        if len(self.cut_rectangles) > 0:
            for rect_loc in self.cut_rectangles:
                x, y, w, h = rect_loc[:4]
    ...
    H, W = img.shape[:2]
    im_white = np.zeros((H, W, 3), dtype=np.uint8)
    for i in range(3):
        im_white[:, :, i] = img
    im = Image.fromarray(im_white)
    delete_end_points_index = []
    print_flags = False # for debugging purpose
    if print_flags:
        print('step', cut_number)
    for j, x in enumerate(end_points):
        mid_point, angle_deg, width, height = get_serial_detect_values(x)
        ...
        for idx, vert in enumerate(vertical_sum):
            if vert > 0.90 * vert_max:  # 준형: 0.88 -> 0.90
        ...
        # 준형
        # error: vertical sum of pixels maximum value too low
        if vert_max < (29/32) * (32 * 255):
            if print_flags:
                print('f7')
            is_screw = True
            delete_end_points_index.append(j)
        ...
        for k in range(len(cut_index) - 1):
            ...
            try:
                ...
                if SAVE_SERIAL:
                    cv.imwrite(os.path.join(self.opt.serial_path, self.opt.assembly_name + '_' + str(cut_number).zfill(2) + '_' + str(j).zfill(2) + '_' + str(k).zfill(2) + '.png'), img_indiv)
            except:
                pass
        ...
        if SAVE_SERIAL_WHOLE and is_screw == False:
            cv.imwrite(os.path.join(self.opt.serial_whole_path, self.opt.assembly_name + '_' + str(cut_number).zfill(2) + '_' + str(j).zfill(2) + '.png'), im_cropped)
    ...
    if SAVE_SERIAL_BLACK:
        for x in end_points:
            ...
            pass
        cv.imwrite(os.path.join(self.opt.serial_black_path, self.opt.assembly_name + '_' + str(cut_number).zfill(2) + '.png'), img)

    # SAVE serial white
    if SAVE_SERIAL_WHITE:
        cv.imwrite(os.path.join(self.opt.serial_white_path, self.opt.assembly_name + '_' + str(cut_number).zfill(2) + '.png'), im_white)  # red # serial region
...
def mult_detect(self, cut, cut_number):
    """
    ...
    ret = c.shape[0]
    if SAVE_MULT_BLACK:
        cv.imwrite(os.path.join(self.opt.mult_black_path, self.opt.assembly_name + '_' + str(cut_number).zfill(2) + '_00.png'), img_cc_with_num)
    ...
    for meth in methods:
        ...
        if SAVE_MULT_BLACK:
            cv.imwrite(os.path.join(self.opt.mult_black_path, self.opt.assembly_name + '_' + str(cut_number).zfill(2) + '_01.png'), res)
        ...
        for r1 in range(1, ret1):
            ...
            if diff[index] < 40:
                ...
                if SAVE_MULT_BLACK:
                    cv.imwrite(os.path.join(self.opt.mult_black_path, self.opt.assembly_name + '_' + str(cut_number).zfill(2) + '_02.png'), img_cc_with_num)
                ...
                try:
                    ...
                    if SAVE_MULT_BLACK:
                        cv.imwrite(os.path.join(self.opt.mult_black_path, self.opt.assembly_name + '_' + str(cut_number).zfill(2) + '_03.png'), img_cc_with_num)
                    if points.shape[0] == 8:
                        ...
                        if SAVE_MULT:
                            cv.imwrite(os.path.join(self.opt.mult_path, self.opt.assembly_name + '_' + str(cut_number).zfill(2) + '_' + str(mult_num_order).zfill(2) + '_'
                                                + '00.png'), padded_0)
                        ...
                        # mult_images_list.append(padded_0)
                        ...
                        mult_images_list.append([padded_0, padded_1])
                        mult_num_order += 1
                    else:
                        ...
                        mult_images_list.append([padded])
                        mult_num_order += 1
                    # x, y, w, h = stats[index, :4]                             ## JH
                    x, y, w, h = x_min, y_min, x_max-x_min, y_max-y_min         ## JH
                    cut_mult_loc.append([int(x), int(y), int(w), int(h)])
                except:
                    pass
    ...
    # return images
    cut_mult_images = mult_images_list

    """
    cut_mult_images: [[img_mult0_0, ...],[img_mult1_0, ...], ...]
    cut_mult_loc : [[loc_mult0], [loc_mult1], ...]  # loc_mult = x, y, w, h (of 'X' sign)
    """
    # return step_mult_images, cut_mult_loc
    return cut_mult_images, cut_mult_loc
