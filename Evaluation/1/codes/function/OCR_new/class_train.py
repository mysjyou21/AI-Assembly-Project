...
def train(args=None):
    ...
    imgs = np.load(args.class_train_img)
    labs = np.load(args.class_train_label)
    print(imgs.shape)
    print(labs.shape)
    num_imgs = len(imgs)
    num_test = 96
    ...
    with tf.Session() as sess:
        train_writer = tf.summary.FileWriter(args.class_weight_path)
        sess.run(init_op)
        avg_loss = 0.
        avg_acc = 0.
        step = 0
        ...
        for ep in range(num_epochs):
            ...
            print('Epoch [%03d] Loss: %.8f Acc: %.3f time: %.3f' %
            ...
            if not ep % 10:
                ...
                print('[*] Eval loss: %.8f, acc: %.3f' % (loss_val, acc_val))
                saver.save(sess,
                           os.path.join(args.class_weight_path, 'model.ckpt'),
                           global_step=step)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--class_weight_path', default='./weight/')
    parser.add_argument('--gpu', default='1')
    parser.add_argument('--class_train_img', default='./npy/img.npy')
    parser.add_argument('--class_train_label', default='./npy/lab.npy')
