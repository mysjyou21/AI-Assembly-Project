...
def draw_bbox(img, loc, prob, clsName, color):
    # loc: (x, y, w, h)
    cv.rectangle(img, (loc[0], loc[1]), (loc[0] + loc[2], loc[1] + loc[3]), color, 4)
    textLabel = '{}: {}'.format(clsName, int(100 * prob))
    (retval, baseLine) = cv.getTextSize(textLabel, cv.FONT_HERSHEY_COMPLEX, 1, 1)
    textOrg = (loc[0], loc[1] - 0)
    cv.rectangle(img, (textOrg[0] - 5, textOrg[1] + baseLine - 5),
...
def get_editdistance(str1, str2):
    ...
    if max_len == 0:
        ...
        dis = ed.distance(list1, list2) / max_len
        return dis


def iou_serial(list1, list2):
    """
    Calculate iou between two general-shape serial bboxes.
    :param list1, list2: [x1, y1, x2, y2, x3, y3, x4, y4]
    :return iou: iou between list1 and list2
    """
    xmin, ymin = 10000, 10000
    xmax, ymax = 0, 0

    x1, y1, x2, y2, x3, y3, x4, y4 = list1
    xmin, ymin = np.min((xmin, x1, x2, x3, x4)), np.min((ymin, y1, y2, y3, y4))
    xmax, ymax = np.max((xmax, x1, x2, x3, x4)), np.max((ymax, y1, y2, y3, y4))

    x1, y1, x2, y2, x3, y3, x4, y4 = list2
    xmin, ymin = np.min((xmin, x1, x2, x3, x4)), np.min((ymin, y1, y2, y3, y4))
    xmax, ymax = np.max((xmax, x1, x2, x3, x4)), np.max((ymax, y1, y2, y3, y4))

    H, W = ymax - ymin, xmax - xmin
    x1, y1, x2, y2, x3, y3, x4, y4 = list1
    poly1 = [[x1-xmin, y1-ymin], [x2-xmin, y2-ymin], [x3-xmin, y3-ymin], [x4-xmin, y4-ymin]]
    x1, y1, x2, y2, x3, y3, x4, y4 = list2
    poly2 = [[x1-xmin, y1-ymin], [x2-xmin, y2-ymin], [x3-xmin, y3-ymin], [x4-xmin, y4-ymin]]

    canvas1 = np.zeros((H, W, 3), dtype=np.uint8)
    canvas2 = np.zeros((H, W, 3), dtype=np.uint8)
    canvas1 = cv.fillConvexPoly(canvas1, points=np.array(poly1), color=(1, 0, 0))
    canvas2 = cv.fillConvexPoly(canvas2, points=np.array(poly2), color=(1, 0, 0))
    canvas1 = (canvas1[:, :, 0]).astype(bool)
    canvas2 = (canvas2[:, :, 0]).astype(bool)

    intersection = np.sum((canvas1 & canvas2).astype(int))
    union = np.sum((canvas1 | canvas2).astype(int))
    iou = intersection / union
    return iou


def iou_mult(list1, list2):
    """
        Calculate iou between two general-shape mult bboxes.
        :param list1, list2: [x1, y1, x2, y2]
        :return iou: iou between list1 and list2
    """
    xmin, ymin = 10000, 10000
    xmax, ymax = 0, 0
    ...
    x1, y1, x2, y2 = list1
    xmin, ymin = np.min((xmin, x1, x2)), np.min((ymin, y1, y2))
    xmax, ymax = np.max((xmax, x1, x2)), np.max((ymax, y1, y2))
    ...
    H, W = ymax - ymin, xmax - xmin
    x1, y1, x2, y2 = list1
    poly1 = [[x1 - xmin, y1 - ymin], [x2 - xmin, y2 - ymin]]
    x1, y1, x2, y2 = list2
    poly2 = [[x1 - xmin, y1 - ymin], [x2 - xmin, y2 - ymin]]
