...
class pascalvoc():
    ...
    def __init__(self, args):
        ...
        self.args = args
        self.mode = args.mode in ['test', 'pose_unit_test', 'pose']
        self.errors = []
        self.imgSize = (0, 0)

        self.gtfolder = args.retrieval_gt_path
        self.detfolder = args.retrieval_intermediate_results_path
    ...
    def pascalvoc_calculate_iou(self):
        ...
        detections, for_draw_bbox = evaluator.PlotPrecisionRecallCurve(
            ...
            savePath=None,
            showGraphic=False)
        # for_draw_bbox[0][-1] = 'FP' # temp blue

        print('---------------')
        if self.mode:
            f = open(self.output_path + '/output.txt', 'w')
        ...
        for metricsPerClass in detections:
            ...
            if totalPositives > 0:
                ...
                if self.mode:
                    print('AP: %s (%s)' % (ap_str, cl), file=f)

        mAP = self.acc_AP / self.validClasses
        mAP_str = "{0:.2f}%".format(mAP * 100)
        if (not self.args.mode=="test") or (self.mode and self.args.output_visualization):
            print('mAP: %s' % mAP_str)
        if self.mode:
            print('mAP: %s' % mAP_str, file=f)
            f.close()
        return mAP * 100, for_draw_bbox
