...
def _weight(name, shape, regularizer=None):
    var = tf.get_variable(name, shape=shape, dtype=tf.float32, initializer=layers.xavier_initializer(uniform=False), regularizer=regularizer)
...
def _norm(norm, x, is_train=True, group_num=32, momentum=0.99, epsilon=0.001):
    if norm is None:
        ...
        norm = layers.instance_norm(x, scope='instance_norm') #reuse..?how..?
    elif norm == 'group':
        norm = layers.group_norm(x, group=group_num, scope='group_norm') #reuse..?
