...
def action_checker(I, action_path, image_order=0, highlight=None):
    # TODO: 은지
    """
                ...
                if (numbers > 10 and num_feature > 2) or (num_feature <= 2):
                    ...
                    if not num in prev_num:
                        ...
                        if pos_check == 0 and x_check:
                            ...
                            cv.imwrite(action_path + '/' + '%d_%d.png' % (image_order, order),
...
#        print('num_x', num_xs / num_n, 'num_y', num_ys / num_n, 'w', W, 'h', H, (num_xs / num_n) / W, (num_ys / num_n) / H)
    if c == 0:
        loc.append([c, x, y, h, w])
    cv.imwrite(action_path + '/bin_%d.png' % image_order, (bin_pos * 255).astype(np.uint8))
    cv.imwrite(action_path + '/pred_%d.png' % image_order, I)

    return loc


def map_action(self, circle_labs, circle_nums, act_dic, step_num):
    """ input: material numbers in a circle, output: corresponding action(s) """
    circle_action = []
    circle_num = []
    serial_list = [['100001'], ['101350'], ['122620'], ['104322'], ['122925']]
    ...
    for serial in serials:
        ...
        circle_tools.append('' if len(act) == 1 else act[1])  # necessary ?
    ...
    return serials, circle_action, circle_num
