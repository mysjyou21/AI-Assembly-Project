...
def circle_detect(self, I, index, param1=80, param2=83, ratio_threshold=0.2):
    # TODO: 준형
    """
    ...
    Ig_c = np.copy(Ig)
    Ig_c = cv.threshold(Ig_c, 140, 255, cv.THRESH_BINARY_INV)[1]

    # Connected Components
    ret, labels, stats, c = cv.connectedComponentsWithStats(Ig_c, connectivity=8)
    delete_index = []
    ...
    for r in range(1, ret):
        ...
        if cc_area < 100:  # yellow
            # too small
            delete_condition = True
        if cc_w < 50 and cc_h < 50:  # yellow
            # too short
            delete_condition = True
    ...
    show(Ig)
    Ig_d = (np.isin(labels, delete_index) * 255).astype(np.uint8)
    kernel = np.ones((3, 3), np.uint8)
    Ig_d = cv.dilate(Ig_d, kernel, iterations=2)
    show(Ig_d)
    Ig = np.clip(Ig.astype(np.float32) + Ig_d.astype(np.float32), 0, 255).astype(np.uint8)
    show(Ig)
    ...
    if cir_coord_ is not None:
        ...
        for n in range(len(cir_coord_)):
            ...
            x1_ = int(np.around(max(0, x_ - r_)))  # - pad
            y1_ = int(np.around(max(0, y_ - r_)))  # - pad
    ...
    for n in range(len(cir_coord)):
        margin = 1.1
        cv.circle(group_img, tuple(cir_coord[n][:2].astype(np.int)), int(margin * cir_coord[n][2]), (255, 255, 255), -1)
    ...
    for n in range(len(cir_coord)):
        # cc index numbers of center points of circles
        group_info[n] = labels[int(cir_coord[n][1]), int(cir_coord[n][0])]
    if len(group_info) != 0:
        for n in range(len(loc)):
            # add group index to loc
            loc[n].append(int(group_info[n]))

    # exclude group with elements more than 5
    u, u_inverse, u_count = np.unique(group_info, return_inverse=True, return_counts=True)
    append_list = []
    for i in range(len(u_count)):
        if u_count[i] <= 5:
            append_list.append(i)
    filter_list = [i for i in range(len(u_inverse)) if u_inverse[i] in append_list]
    loc = [loc[i] for i in filter_list]
    g = [loc[i][-1] for i in range(len(loc))]
    u, u_inverse = np.unique(g, return_inverse=True)
    u_inverse = u_inverse + 1
    ...
    cv.imwrite(os.path.join(self.opt.circlepath, f'circle_{index}.png'), img)
